/**
 * Pre-launch: overwrite RETURNS Version 1 in place for sk/en/hu/de/cs/uk.
 * Does NOT create Version 2. Does not touch TERMS / PRIVACY / COOKIES.
 *
 * Usage: npx tsx scripts/returns-v1-production-upsert.ts
 */
...
import { RETURNS_PRODUCTION_V1 } from '../src/legal/returns-production-v1/content'
...
async function main() {
  ...
  const backupDir = resolve(__dirname, `../backups/legal/returns-v1-pre-upsert-${stamp}`)
  mkdirSync(backupDir, { recursive: true })

  const document = await prisma.legalDocument.findUnique({
    where: { type: LegalDocumentType.RETURNS },
  })
  if (!document) {
    throw new Error('RETURNS LegalDocument row missing — seed the legal module first')
  }
  ...
  for (const entry of RETURNS_PRODUCTION_V1) {
