/**
 * Pre-launch: overwrite TERMS Version 1 in place for sk/en/hu/de/cs/uk.
 * Does NOT create Version 2.
 *
 * Usage: npx tsx scripts/terms-v1-production-upsert.ts
 */
...
import { TERMS_PRODUCTION_V1 } from '../src/legal/terms-production-v1/content'
...
async function main() {
  ...
  const backupDir = resolve(__dirname, `../backups/legal/terms-v1-pre-upsert-${stamp}`)
  ...
  if (!document) {
    throw new Error('TERMS LegalDocument row missing — seed the legal module first')
  }
  ...
  for (const entry of TERMS_PRODUCTION_V1) {
  ...
  const v2 = after.filter((r) => r.version >= 2)
  console.log(
    JSON.stringify(
      {
        ok: true,
  ...
  // sanity: seed content file still present
  void readFileSync(resolve(__dirname, '../src/legal/terms-production-v1/content.ts'))
}
