/**
 * Pre-launch: overwrite PRIVACY Version 1 in place for sk/en/hu/de/cs/uk.
 * Does NOT create Version 2. Does not touch TERMS / RETURNS / COOKIES.
 *
 * Usage: npx tsx scripts/privacy-v1-production-upsert.ts
 */
...
import { PRIVACY_PRODUCTION_V1 } from '../src/legal/privacy-production-v1/content'
...
async function main() {
  ...
  const backupDir = resolve(__dirname, `../backups/legal/privacy-v1-pre-upsert-${stamp}`)
  ...
  if (!document) {
    throw new Error('PRIVACY LegalDocument row missing — seed the legal module first')
  }
  ...
  for (const entry of PRIVACY_PRODUCTION_V1) {
